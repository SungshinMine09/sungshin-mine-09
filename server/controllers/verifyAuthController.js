...
async function checkChatRoomByUserID(req, res, next) {
  const userToken = req.cookies["userToken"];
  let decodedToken = jwt.verify(userToken, secretObj.secret);
  ...
  if (decodedToken) {
    ...
    const CoBuyingRoomID = req.params.id;
    const CoBuyRoom = await Cobuying_room.findByPk(CoBuyingRoomID);
    const current_user = checkID(req);

    if (CoBuyRoom.host_id == decodedToken.db_id) {
      next();
    } else if (login_id && current_user.login_id == login_id) {
      next();
    } else {
      res.send("<script>alert('잘못된 접근입니다.'); history.back(); </script>");
    }
...
module.exports = { checkAuth, checkID, checkSeller, checkSellerbyID, checkSellerbyRoomID, checkSellerbyFormID, checkChatRoomByUserID };
